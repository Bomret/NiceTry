...
namespace NiceTry {
    static class _ {
        ...
        public static void ThrowIfNullOrInvalid<T>([NotNull] this Try<T> @try, string name) {
            @try.ThrowIfNull(nameof(@try));
            if(@try.IsSuccess == @try.IsFailure)
                throw new ArgumentException("The specified Try<T> does not represent either success or failure or represents both at once.", name);
        }
